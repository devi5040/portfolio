      ...
      >
        ...
        <div className="max-w-7xl mx-auto p-6">
          <div className="relative">
            <div className="absolute left-0 md:left-1/2 top-0 h-full w-0.5 bg-neutral -translate-x-1/2" />
            {PROFESSIONAL_EXPERIENCES.map((item, index) => (
              <div
                key={index}
                className={`relative flex items-center ${index % 2 == 0 ? 'justify-start' : 'justify-end'}`}
              >
                <div className="absolute -left-5 md:left-1/2 md:-translate-x-1/2 w-10 h-10 rounded-full bg-background border border-white/10 flex items-center justify-center z-10 shadow-[0_0_15px_rgba(168,85,247,0.4)]">
